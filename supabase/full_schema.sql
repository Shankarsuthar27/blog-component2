...
create policy "Allow public read published news_articles" on public.news_articles for select using (status = 'published');
create policy "Allow full news_articles access for admins" on public.news_articles for all using (true);
create policy "Allow full news_import_logs access" on public.news_import_logs for all using (true);
...
returns void as $$
begin
    update public.news_articles set views = views + 1 where id = p_article_id;
end;
$$ language plpgsql security definer;
